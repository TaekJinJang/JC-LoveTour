...
function header() {
  const [showMenu, setShowMenu] = useState(false); // 마우스 호버 상태를 저장하는 상태값
    return (
<Container style={{backgroundImage: `url(${background})`, height: '400px', backgroundSize: 'cover', backgroundRepeat: 'no-repeat', backgroundPosition: 'center center'}}>

  <Row>
    <Col style={{ height: '70px' }}></Col>
  </Row>
  ...
  <Row>
    <hr style={{
        ...
        onMouseLeave={() => setShowMenu(false)} // 네비게이션에서 마우스를 내렸을 때 상태값 변경
          ...
          <Col md='auto'>
            <Nav.Item as='li'>
            <Nav.Link href="/">제천 러브투어</Nav.Link>
        ...
        <Row>
          <Col className="text-center" style={{ height: '350px' , display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
          <div style={{color: 'white' , textAlign: 'center', zIndex: '10', position: 'absolute', top: '300px', left: '50%', transform: 'translateX(-50%)'}}>
            <h2>제천여행</h2>
